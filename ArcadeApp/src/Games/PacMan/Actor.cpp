...
void Actor::Init(const SpriteSheet& spriteSheet, const std::string& animationsPath, const Vec2D& initialPos, int movementSpeed, bool updateSpriteOnMovement, const Color& spriteColor)
{
...
void Actor::Update(int dt)
{
	if (mMovementDirection != PACMAN_MOVEMENT_NONE)
	{
		...
		{
			int dx = static_cast<int>(fabsf(mDelta.GetX()));
			if (mDelta.GetX() < 0)
			{
		...
		{
			int dy = static_cast<int>(fabsf(mDelta.GetY()));
			if (mDelta.GetY() < 0)
			{
