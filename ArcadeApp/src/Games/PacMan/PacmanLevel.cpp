...
bool PacmanLevel::LoadLevel(const std::string& levelPath)
{
	...
	bgImageCommand.parseFunc = [this, &bgImageName](ParseFuncParams params) {
		...
		assert(loaded && "Couldn't load background image");
	};
	...
	layoutCommand.parseFunc = [&layoutOffset, this](ParseFuncParams params)
	{
		...
		for (int c = 0; c < params.line.length(); ++c)
		{
			...
			if (tile)
			{
				...
				{
					mGhostSpawnPoints[BLINKY] = Vec2D(startingX + tile->offset.GetX() + 5, layoutOffset.GetY() + tile->offset.GetY());
				}
				...
				{
					mGhostSpawnPoints[INKY] = Vec2D(startingX + tile->offset.GetX() + 1, layoutOffset.GetY() + tile->offset.GetY());
				}
